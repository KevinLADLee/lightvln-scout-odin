...
static void lidar_device_callback(const lidar_device_info_t* device, bool attach)
{
    ...
    if(attach == true) {
        ...
        const std::string config_dir =
            odin_ros_driver::GetOdinRuntimeDir() + "/config";
        #ifdef ROS2
            RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                        "========== [CALIB] SAVE DIR (primary): %s | BACKUP DIR: %s ==========",
                        odin_ros_driver::GetOdinCalibDir().c_str(), config_dir.c_str());
        #else
        ...
        std::string calib_runtime_dir = odin_ros_driver::GetOdinCalibDir();
...
int main(int argc, char *argv[])
{
    ...
    try {
        ...
        const std::string runtime_dir = odin_ros_driver::GetOdinRuntimeDir();
        const char* custom_data_dir = std::getenv("ODIN_DATA_DIR");
        const std::string data_dir = custom_data_dir && *custom_data_dir
            ? std::string(custom_data_dir)
            : runtime_dir + "/recorddata";
        const std::string log_dir = runtime_dir + "/log";
        const std::string map_dir = runtime_dir + "/map";

...
// Single source of truth for writable runtime paths, shared by the writer
// (host_sdk_sample) and every reader (cloud_reprojection, pcd2depth).
// Header-only inline functions avoid multiple-definition across translation units.
namespace odin_ros_driver {

// Resolve the writable directory for logs, maps, and recordings.
// Priority: ODIN_RUNTIME_DIR -> ROS_HOME -> ~/.ros -> /tmp.
// Works for both ROS1 and ROS2 (~/.ros is the shared default ROS_HOME).
inline std::string GetOdinRuntimeDir() {
    if (const char* custom = std::getenv("ODIN_RUNTIME_DIR")) {
...
// ODIN_CALIB_DIR remains a focused override for deployments that keep device
// calibration separate from the other runtime artifacts.
inline std::string GetOdinCalibDir() {
    if (const char* custom = std::getenv("ODIN_CALIB_DIR")) {
        return std::string(custom);
    }
    return GetOdinRuntimeDir();
}

// Canonical calib.yaml path shared by writer and all readers.
inline std::string GetOdinCalibFile() {
    return GetOdinCalibDir() + "/calib.yaml";
}

...
Writable runtime data is stored outside the source tree under
`~/.ros/odin_ros_driver/` by default. It contains `calib.yaml`, `config/`,
`recorddata/`, `log/`, and `map/`. Override the common root with
`ODIN_RUNTIME_DIR`; `ODIN_CALIB_DIR` and `ODIN_DATA_DIR` provide narrower
overrides for calibration and recordings.

(() => {
  ...
  const state = {
    ...
    previewStamp: "", previewSource: "camera_preview",
  ...
  async function loadCameraFrame() {
    ...
    try {
      ...
      state.previewStamp = response.headers.get("X-Capture-Stamp") || "";
      state.previewSource = response.headers.get("X-Frame-Source") || "camera_preview";
      if (ui.image.naturalWidth && ui.image.naturalHeight) {
        ui.image.parentElement.style.aspectRatio = `${ui.image.naturalWidth} / ${ui.image.naturalHeight}`;
        if (ui.overlay.width !== ui.image.naturalWidth || ui.overlay.height !== ui.image.naturalHeight) {
          ui.overlay.width = ui.image.naturalWidth;
          ui.overlay.height = ui.image.naturalHeight;
        }
      }
      drawWaypoints();
  ...
  function render() {
    ...
    setPill(ui.camera, cameraOnline, false, state.previewSource === "vln_input" ? "VLN INPUT" : "CAMERA");
  ...
  function drawWaypoints() {
    const canvas = ui.overlay, width = canvas.width, height = canvas.height, context = canvas.getContext("2d");
    context.clearRect(0, 0, width, height);
    if (!state.vln.enabled || state.previewSource !== "vln_input"
      || !state.previewStamp || state.previewStamp !== state.vln.capture_stamp) return;
    ...
    const drawSemanticMarker = (pixel, channel, semanticState, color) => {
      ...
      const frameSize = state.vln.frame_size;
      const sourceWidth = Array.isArray(frameSize) ? Number(frameSize[0]) : width;
      const sourceHeight = Array.isArray(frameSize) ? Number(frameSize[1]) : height;
      if (!(sourceWidth > 0 && sourceHeight > 0)) return;
      const point = {x:Number(pixel[0]) * width / sourceWidth, y:Number(pixel[1]) * height / sourceHeight};
